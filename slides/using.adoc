...
MessageBatch
------------
* Contains an array of messages *from the same sender*
* Only created on the receiver side
* Example:
...
* Users typically extend `ReceiverAdapter` and override `receive(Message)` / `receive(MessageBatch)` and `viewAccepted()`:
...
public void receive(MessageBatch batch) { // <1>
...
----
<1> The default implementation of `receive(MessageBatch)` calls `receive(Message)` for each message in the batch
...
* Set `-Dcom.sun.management.jmxremote` to enable JMX
