...
* `RELAY3`
** Reliability across sites: M will get retransmitted when the new site master of A (or B) comes up
*** This applies only to *unicast* messages
** Asymmetric routing: not all sites are connected to each other: to send a message to a site Y, it might have to get sent
  via (gateway) site X
* Example:
...
<relay.RELAY3 site="net1"
    ...
    <RelayConfiguration xmlns="urn:jgroups:relay:1.0"
        ...
        <sites>
            <site name="net1">
                <bridges>
                    <bridge name="bridge-net1-net2" config="bridge-net1-net2.xml"/>
                    <bridge name="bridge-net1-hf"   config="bridge-net1-hf.xml"/>
                </bridges>
                <forwards>
...
----
* The above configuration defines site `net1`
* There are 2 bridges (= global clusters) between site `net1` and `hf`, and `net1` and `net2`. When those bridges are
  started, the routing tables will be adjusted automatically, e.g.:
** Site master `_n2:net2` joins the cluster -> this adds `_n2:net2` to the routes for site `net2`
* The `<forward>` defines that all messages to site `net3` should be sent to the site master of site `net2`, which
  forwards them to site `net3`.
* Demo
...
== Outlook / todos
* IspnPerfTest JGroups `5.5.1` Infinispan `16.Dev08`, comparing existing versus new protocols
* TBD: use new protocols in Infinispan
* TBD: use `RELAY3` in Infinispan
* InfinispanMessage: implement and perf test
