...
Message.RSVP: When set, a message send will block until the receiver (unicast) or receivers
...
[source,java]
----
ch.send(new Message(null, "hello world").setFlag(Message.RSVP));
----
...
public synchronized void register(boolean register) {
	...
	try {
		if(register && !is_registered) {
		    JmxConfigurator.registerChannel(ch), Util.getMBeanServer(), tp.getClusterName());
		    is_registered = true;
		}
		else {
		    if(is_registered) {
		        JmxConfigurator.unregisterChannel(ch, Util.getMBeanServer(), tp.getClusterName());
			    is_registered = false;
		    }
		}
	} catch (Exception e) {
