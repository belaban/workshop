...
Regular thread pool
-------------------
image::../images/RegularThreadPool.svg[Regular thread pool,width="100%"]



OOB thread pool
---------------
image::../images/OOBThreadPool.svg[OOB thread pool,width="100%"]
...
Lab: advanced
-------------
* Run 1 instance of `bin/advanced.sh`
* Press `[1] start`: the first RPC takes 1s, the second 2s, then 3s, 4s, 5s. All subsequent RPCs take 5s. Why ?
* All RPCs from A are invoked _sequentially_ (5 different sender threads)
* That's the semantics of regular messages: all reg messages are delivered in the order in which they were sent
