...
|====
| Name | Implementation
| UDP | Sends datagram packets (to single destination) or IP multicast packets (to all cluster nodes). The latter requires IP multicasting to be enabled.
| TCP | Uses TCP sockets. A message sent to the entire cluster (N nodes) is sent N-1 times. 1 thread per connection.
| TCP_NIO2 | Same as TCP, but uses NIO.2: thread pool to handle _all_ connections.
...
* Used to find the *coordinator*  (to send a `JOIN` request to it)
* Implementations:

[width="100%",cols="2,10", frame="topbot",options="header"]
|====
| Name | Implementation
| PING | Sends discovery request as multicast (requires `UDP` as transport)
...
| S3_PING, GOOGLE_PING| Uses a cloud store for membership information. More impls include AWS_PING,
                        RACKSPACE_PING, Azure, Kubernetes etc
|DNS_PING| Uses DNS to look up members
...
Merging
--------
* When failure detection splits a cluster into multiple subclusters, e.g. `A15={A,B,C}` and `D15={D,E}`,
  merging tries to merge the subclusters back into a single cluster:
...
Failure detection
-----------------
* Failure detection checks for crashed or unresponsive members and *suspects* them
** It *doesn't remove suspected members* (that's the task of `GMS`)
...
|====
| Name | Implementation
| FD_ALL | Every node periodically multicasts (= 'send to all') heartbeats. Members maintains a table of members and
           timestamps and reset the timestamp for P on reception of a heartbeat or message from P. +
...
| FD | Same as `FD_SOCK` but instead of a TCP connection, heartbeat messages are sent to the neighbor to the right. If
       ...
       *Not recommended anymore as the detection time of a crash of multiple adjacent members is big.*
...
Group membership
----------------
* Handles new member joining, existing members leaving or crashing (suspicions)
...
* Senders and receivers start out with *credits* which are bytes to send
...
| UFC | Unicast flow control. Maintains flow control between 2 peers for unicast messages. Can be removed if `TCP` is
        used as transport. +
        Blocks the sender in case of insufficient credits, until more credits have been received.
| MFC | Multicast flow control. Maintains a credit table for _all_ receivers and senders and blocks for credits
        ...
        Blocks the sender in case of insufficient credits, until more credits have been received.
| UFC_NB | Non-blocking version of `UFC`. When a message cannot be sent due to insufficient credits, instead of blocking
           the sender, the message is queued and sent later, when credits have been received. The queue can be bounded
           to prevent it from growing endlessly; this would again block senders. +
           *In combination with TCP_NIO2, a completely non-blocking stack can be configured*.
| MFC_NB | Non-blocking version of MFC.
|====
...
| FRAG3 | A more memory-conserving implementation
|====
...
| SYM_ENCRYPT | Encryption on the sender and decryption on the receiver(s), using a shared key (from a keystore)
| ASYM_ENCRYPT | Same as `SYM_ENCRYPT`, but the shared key is fetched on startup from the key server (coordinator).
                 Can be configured to use an external key exchange protocol.
|SSL_KEY_EXCHANGE| External key exchange protocol using SSL sockets and certificates. Can be used by `ASYM_ENCRYPT` to
...
|====
| Name | Implementation
| RELAY2 | Relaying of messages between separate clusters.
